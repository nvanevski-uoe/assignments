 ...
 * Uses exchangerate.host which does not require an API key.
 */
public final class CurrencyConverter {
    private static final String API_ENDPOINT = "https://api.exchangerate.host/convert";

    private static final Pattern RESULT_PATTERN = Pattern.compile("\\\"result\\\"\\s*:\\s*([-+]?[0-9]*\\.?[0-9]+(?:[eE][-+]?[0-9]+)?)");
    ...
    public BigDecimal convert(String from, String to, BigDecimal amount) throws IOException, InterruptedException {
        ...
        String url = String.format(Locale.ROOT, "%s?from=%s&to=%s&amount=%s", API_ENDPOINT,
        ...
        BigDecimal result = parseResult(body);
        if (result == null) {
    ...
    private static BigDecimal parseResult(String json) {
